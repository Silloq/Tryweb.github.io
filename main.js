...
// Initialize the camera with the above parameters
...
const texture = textureLoader.load("Imagges/Chpic.jpg", (texture) => {
  texture.minFilter = THREE.LinearFilter;
  texture.magFilter = THREE.LinearFilter;
  const circleShape = new THREE.Shape();
  circleShape.moveTo(0, 0); // Move to center of the circle
  circleShape.absarc(0, 0, 4, 0, Math.PI * 2, false); // Create a circle shape
  
  // Extrude the circle shape to give it depth
  const radius = 4;  
  ...
  function animate() {
    ...
    plane.rotation.y += 0.01; // Rotate the plane along the y-axis
